...
| Paths that were never exercised | The evaluation script and the cross-reward matrix were written, reviewed, committed -- and never run. The first crashed on its first invocation because it cached the baseline's completions *inside the baseline checkpoint directory*, which fails when the baseline is a model id and pollutes the artifact when it is a path. 22 evaluation jobs were queued against it. Smoke-run every path before scheduling work on it. |
| A memory search that leaks | `empty_cache()` frees cached blocks, not referenced ones, and a raised exception keeps every frame's locals alive through its traceback -- including the model. Each autofit attempt inherited its predecessor, so the search reported "does not fit at any size" when it had measured a leak. The symptom was PPO failing to allocate **18 MiB**. Bench functions now `del` their model before re-raising, and the search reports how much actually came back. |
