...
RLAIF judge & \texttt{Qwen/Qwen2.5-3B-Instruct}, read from logits \\
Self-judge & frozen copy of the SFT checkpoint \\
\jev{} & \texttt{typesafe/jev-1.13} \\
Evaluation judge & \texttt{openai/gpt-5-mini} (unrelated to every training judge) \\
GRPO & $G=8$, 16 prompts/step, 180 steps, lr $10^{-6}$, $\beta=0.04$ \\
PPO & 64 prompts/step, 110 steps, lr $10^{-6}$, KL coef 0.05, GAE $\lambda=0.95$ \\
Reward-call budget & 23,040 per GRPO run (matched across sources) \\
