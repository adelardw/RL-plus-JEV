...
\input{tables/judge_benchmark.tex}
...
Second, the efficiency claim, stated carefully because the obvious version of
it is not defensible. \input{tables/efficiency_inline.tex}\unskip{} That ratio
compares two \emph{deployment modes}, not two models, so we swept both sides
rather than quoting it: \jev{} across concurrency and the local judge across
batch size (Table~\ref{tab:throughput}).

The sweep did not go as expected. The local judge's throughput \emph{falls}
monotonically as its batch grows --- 0.72, 0.61 and 0.51 judgements per second
at batches of 4, 8 and 16 --- while its memory rises from 4.3 to 8.5\,GB, and a
batch of 32 runs out of memory. The cause is structural rather than incidental:
a rubric of $Q$ questions is $Q$ separate prefills over a shared state, so a
larger batch mostly buys padding. Batching is the obvious lever and it points
the wrong way. We therefore compare each side at its own best setting, which is
the comparison that favours the local judge as far as it can be favoured.

What does not depend on tuning is where the computation has to happen. During
RL the local judge occupies the same accelerator as the policy it is scoring:
it cannot overlap with generation, and on the 14.6\,GB cards used here a 7--8B
judge does not fit beside the policy at all --- it had to be sharded across both
GPUs simply to run. A judge reached over the network contends for nothing. This
matters more in the critic role than in the reward role, because a critic is
queried $K$ times per completion rather than once, so the arithmetic that
decides feasibility is $K$ times harsher. We therefore report wall-clock, peak
memory and dollars per judgement and let the reader apply their own hardware,
rather than resting the argument on a single ratio.

\input{tables/judge_throughput.tex}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/prefix_value.pdf}
\caption{Left: the gap each critic holds between prefixes that end well and
prefixes that end badly, as a function of how much of the response it has seen.
The outcome is not observable at the time of the query. All three separate the
two groups before the response ends, which is the claim the critic role rests
on, and the hosted judge separates them most. Right: the fraction of responses
each critic values at exactly 0 or 1. These two panels are in tension --- the
judge with the widest separation is also the one most often pinned at an
extreme --- and Section~\ref{sec:res-calibration} takes that up.}
\label{fig:prefix}
\end{figure}

\paragraph{The mechanism transfers, and the reading we expected does not.}
All three readers answer the prefix question usefully: AUC against the
eventual outcome rises with the fraction of the response seen for each of them,
so the critic role is a property of \emph{readable calibrated belief} rather
than of one vendor's model. That is the claim we set out to test, and it holds.

What does not hold is the reading we expected of the second panel. We had
supposed that a saturated critic would be a worse critic, because a
temporal-difference error is a difference between neighbouring values and a
curve pinned at 0 or 1 has none to give. The data do not support that as
stated: the hosted judge saturates on three quarters of responses and yet holds
the \emph{widest} separation and the highest terminal AUC. Saturation and
discrimination are not in conflict --- pushing values to the extremes is what
widens the gap between groups, at the cost of resolution within them.

The question the two panels actually pose is therefore narrower than we
framed it, and we have not answered it here: whether the gradient a critic
supplies \emph{between} probe points matters more to PPO than the separation
it achieves at the end. That is decided by the PPO arms, not by this
measurement, and we resist reading it off a correlation.

\subsection{How fast does the sparse-probe bias vanish?}
\label{sec:res-density}
\input{tables/probe_density.tex}

\begin{figure}[t]
\centering
\includegraphics[width=0.55\linewidth]{figures/probe_density.pdf}
\caption{Interpolation error against the number of probes, measured against a
densely probed value curve, with the two rates of Proposition~\ref{prop:bias}
anchored at $K=2$ for slope comparison. The mean error tracks the Lipschitz
rate; the worst case does not, which is the signature of a value curve that
steps rather than bends.}
\label{fig:density}
\end{figure}
