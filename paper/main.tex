...
\caption{Left: the gap the critic holds between prefixes that end well and
prefixes that end badly, as a function of how much of the response it has seen.
The outcome is not observable at the time of the query. Right: the fraction of
responses each critic values at 0 or 1. Saturation is harmless for a terminal
reward and damaging for a critic, because a temporal-difference error is a
\emph{difference} between neighbouring values --- a saturated curve carries no
signal in the middle of a response, which is where credit assignment is
needed.}
\label{fig:prefix}
\end{figure}
