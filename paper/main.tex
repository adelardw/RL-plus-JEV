...
compares two \emph{deployment modes}, not two models: \jev{} is queried over the
network at concurrency 48, while the local judge runs on one GPU at a fixed
batch size. Table~\ref{tab:throughput} sweeps that batch size so the part of the
gap that tuning can close is separated from the part that cannot, and we take
the tuned figure as the comparison.
